...
class Stock_model_new extends CI_Model {
    ...
    /**
     * Mark PO as ready for stock addition
     * @param int $po_id
     * @return bool
     */
    public function mark_po_ready_for_stock($po_id) {
        try {
            // Update PO status to indicate it's ready for stock addition
            $this->db->where('id', $po_id);
            $this->db->update('new_purchase_orders', [
                'stock_addition_enabled' => 1,
                'stock_addition_enabled_at' => date('Y-m-d H:i:s'),
                'updated_at' => date('Y-m-d H:i:s')
            ]);
            
            return $this->db->affected_rows() > 0;
        } catch (Exception $e) {
            log_message('error', 'Failed to mark PO ready for stock: ' . $e->getMessage());
            return false;
        }
    }

    /**
     * Get purchase orders ready for stock addition
     * @return array
     */
    public function get_purchase_orders_for_stock_addition() {
        try {
            $this->db->select('npo.*, hv.name as vendor_name');
            $this->db->from('new_purchase_orders npo');
            $this->db->join('hms_vendors hv', 'npo.vendor_number = hv.vendor_number', 'left');
            $this->db->where('npo.status', 'approved');
            $this->db->where('npo.stock_addition_enabled', 1);
            $this->db->where('npo.stock_addition_completed', 0);
            $this->db->order_by('npo.created_at', 'DESC');
            
            return $this->db->get()->result();
        } catch (Exception $e) {
            log_message('error', 'Failed to get POs for stock addition: ' . $e->getMessage());
            return array();
        }
    }
    ...
    public function get_purchase_order_for_stock_addition($po_id) {
        try {
            ...
            $this->db->from('new_purchase_orders npo');
            $this->db->join('hms_vendors hv', 'npo.vendor_number = hv.vendor_number', 'left');
            $this->db->where('npo.id', $po_id);
            $this->db->where('npo.status', 'approved');
            $this->db->where('npo.stock_addition_enabled', 1);
            
            return $this->db->get()->row();
        } catch (Exception $e) {
            log_message('error', 'Failed to get PO for stock addition: ' . $e->getMessage());
            return null;
        }
    }
     ...
     * @return array
     */
    public function get_purchase_order_items_for_stock($po_id) {
        try {
            $this->db->select('npoi.*, m.medicine_name, m.generic_name, m.strength, m.unit, m.category, mb.brand_name');
            $this->db->from('new_purchase_order_items npoi');
            $this->db->join('medicines m', 'npoi.medicine_id = m.id', 'left');
            $this->db->join('medicine_brands mb', 'm.brand_id = mb.id', 'left');
            $this->db->where('npoi.po_id', $po_id);
            $this->db->order_by('npoi.id', 'ASC');
            
            return $this->db->get()->result();
        } catch (Exception $e) {
            log_message('error', 'Failed to get PO items for stock: ' . $e->getMessage());
            return array();
        }
    }

    /**
     * Add stock from purchase order to new stocks module
     * @param int $po_id
     ...
     */
        ...
        try {
            ...
            $po = $this->get_purchase_order_for_stock_addition($po_id);
            if (!$po) {
                throw new Exception('Purchase order not found or not ready for stock addition');
            }
            ...
            foreach ($stock_items as $item) {
                ...
                $medicine = $this->get_medicine_by_id($item['medicine_id']);
                if (!$medicine) {
                    log_message('error', 'Medicine not found: ' . $item['medicine_id']);
                    continue;
                }
                
                // Generate unique batch number
                $batch_number = $this->generate_unique_batch_number($item['medicine_id'], $item['batch_number']);
                
                // Create batch data
                $batch_data = [
                    'medicine_id' => $item['medicine_id'],
                    'vendor_id' => $this->get_vendor_id_by_number($po->vendor_number),
                    ...
                    'batch_status' => 'ACTIVE',
                    'remarks' => 'Added from PO: ' . $po->po_number,
                ...
                if ($batch_id) {
                    ...
                    $this->add_central_stock($central_stock_data);
                    ...
                    $this->log_stock_movement([
                        ...
                        'from_location_id' => $this->get_vendor_id_by_number($po->vendor_number),
                        'to_location_type' => 'CENTRAL',
                        'to_location_id' => 1, // Central warehouse
            ...
            // Mark PO as stock addition completed
            if ($success_count > 0) {
                $this->db->where('id', $po_id);
                $this->db->update('new_purchase_orders', [
                    'stock_addition_completed' => 1,
                    'stock_addition_completed_at' => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d H:i:s')
                ]);
            }
            ...
            log_message('error', 'Failed to add stock from PO: ' . $e->getMessage());
            return false;
        }

    /**
     * Add stock from purchase order directly to a specific center
     * @param int $po_id Purchase order ID
     * @param array $stock_items Stock items data
     * @param int $center_id Target center ID
     * @return int|false Number of successful additions or false on failure
     */
    public function add_stock_from_purchase_order_to_center($po_id, $stock_items, $center_id) {
        try {
            $this->db->trans_start();
            ...
            foreach ($stock_items as $item) {
                ...
                $batch_data = [
                    ...
                    'quality_status' => 'APPROVED',
                    'batch_status' => 'ACTIVE',
                    'remarks' => 'Added from PO: ' . $po->po_number . ' to Center: ' . $center_id,
                    'created_by' => $this->get_current_user_id(),
                    'po_reference' => $po->po_number,
                    'po_id' => $po_id
                ];
                ...
                $batch_id = $this->add_batch($batch_data);
                if ($batch_id) {
                    // Add to center stock directly (skip central stock)
                    $center_stock_data = [
                        'batch_id' => $batch_id,
                        'medicine_id' => $item['medicine_id'],
                        'center_id' => $center_id,
                    ...
                    $this->add_center_stock($center_stock_data);
            ...
            $this->db->trans_rollback();
            log_message('error', 'Failed to add stock from PO to center: ' . $e->getMessage());
            return false;
        }
    }
     ...
     * @param string $base_batch_number
     * @return string
     */
    private function generate_unique_batch_number($medicine_id, $base_batch_number) {
        ...
        while ($this->batch_number_exists($batch_number, $medicine_id)) {
            $batch_number = $base_batch_number . '-' . $counter;
            $counter++;
        }
     ...
     * @return bool
     */
    private function batch_number_exists($batch_number, $medicine_id) {
        $this->db->where('batch_number', $batch_number);
        $this->db->where('medicine_id', $medicine_id);
        return $this->db->count_all_results('medicine_batches') > 0;
    }

    /**
     * Get vendor ID by vendor number
     * @param string $vendor_number
     * @return int|null
     */
    private function get_vendor_id_by_number($vendor_number) {
        $this->db->select('ID');
     ...
     * @return bool
     */
    private function add_central_stock($data) {
     ...
     * @return bool
     */
    private function log_stock_movement($data) {
        return $this->db->insert('stock_movements', $data);
    }

    /**
     * Get batches added from purchase orders
     * @param string $po_number Optional PO number filter
     * @return array
     */
    public function get_po_added_batches($po_number = null) {
        try {
            $this->db->select('mb.*, m.medicine_name, m.medicine_code, mb2.brand_name as brand_name, v.name as vendor_name, DATEDIFF(mb.expiry_date, CURDATE()) as expiry_days, mb.po_reference, mb.po_id');
            ...
            $this->db->where('mb.po_reference IS NOT NULL');
            $this->db->where('mb.po_reference !=', '');
            
            if($po_number && $po_number != '') {
                $this->db->where('mb.po_reference', $po_number);
            }
    ...
    public function get_top_selling_medicines($limit = 10) {
        try {
            $this->db->select('m.medicine_name, mb.brand_name as brand_name, SUM(si.quantity_sold) as total_sold, SUM(si.total) as total_revenue');
    ...
    public function add_medicine_brand($data) {
        return $this->db->insert('medicine_brands', $data);
    }

    public function update_medicine_brand($id, $data) {
        $this->db->where('id', $id);
        return $this->db->update('medicine_brands', $data);
    }

    public function get_medicine_brand_by_id($id) {
        $this->db->where('id', $id);
        return $this->db->get('medicine_brands')->row();
    }
    ...
    public function get_all_batches($medicine_id = null, $vendor_id = null, $batch_number = null, $batch_status = null) {
        ...
        try {
            $this->db->select('mb.*, m.medicine_name, m.medicine_code, mb2.brand_name as brand_name, v.name as vendor_name, DATEDIFF(mb.expiry_date, CURDATE()) as expiry_days, COALESCE(mb.quality_status, "PENDING") as quality_status, mb.po_reference, mb.po_id');
            $this->db->from('medicine_batches mb');
            $this->db->join('medicines m', 'mb.medicine_id = m.id');
            $this->db->join('medicine_brands mb2', 'm.brand_id = mb2.id');
            $this->db->join($this->config->item('db_prefix') . 'vendors v', 'mb.vendor_id = v.ID');
    ...
    public function get_batch_by_id($id) {
        $this->db->select('mb.*, m.medicine_name, m.medicine_code, mb2.brand_name as brand_name, v.name as vendor_name, COALESCE(mb.quality_status, "PENDING") as quality_status, mb.quantity_remaining as current_stock');
        $this->db->from('medicine_batches mb');
        $this->db->join('medicines m', 'mb.medicine_id = m.id');
        $this->db->join('medicine_brands mb2', 'm.brand_id = mb2.id');
        $this->db->join($this->config->item('db_prefix') . 'vendors v', 'mb.vendor_id = v.ID');
    ...
    public function get_central_stocks($medicine_id = null, $batch_number = null, $status = null) {
        try {
            $this->db->select('cs.*, mb.batch_number, mb.expiry_date, mb.purchase_price, mb.selling_price, m.medicine_name, m.medicine_code, b.name as brand_name, v.name as vendor_name, DATEDIFF(mb.expiry_date, CURDATE()) as expiry_days, mb.po_reference, mb.po_id');
    ...
    public function get_transfer_items($transfer_id) {
        $this->db->select('sti.*, m.medicine_name, m.medicine_code, mb.brand_name as brand_name, mb2.batch_number, mb2.expiry_date');
    ...
    public function get_available_batches_for_transfer($center_id, $department = null, $employee_number = null) {
        if($center_id) {
            ...
            $this->db->select('mb.*, m.medicine_name, m.medicine_code, mb2.brand_name as brand_name, ccs.quantity as available_quantity');
            $this->db->from('medicine_batches mb');
            $this->db->join('medicines m', 'mb.medicine_id = m.id');
            $this->db->join('medicine_brands mb2', 'm.brand_id = mb2.id');
            ...
            $this->db->select('mb.*, m.medicine_name, m.medicine_code, mb2.brand_name as brand_name, cs.quantity as available_quantity');
            $this->db->from('medicine_batches mb');
            $this->db->join('medicines m', 'mb.medicine_id = m.id');
            $this->db->join('medicine_brands mb2', 'm.brand_id = mb2.id');
    ...
    public function get_stocks_by_location($center_id, $department, $employee_number) {
        $this->db->select('mb.*, m.medicine_name, m.medicine_code, mb2.brand_name as brand_name, v.vendor_name');
        $this->db->from('medicine_batches mb');
        $this->db->join('medicines m', 'mb.medicine_id = m.id');
        $this->db->join('medicine_brands mb2', 'm.brand_id = mb2.id');
    ...
    public function get_central_batches() {
        $this->db->select('mb.*, m.medicine_name, m.medicine_code, mb2.brand_name as brand_name, v.vendor_name, cs.quantity as available_quantity, DATEDIFF(mb.expiry_date, CURDATE()) as expiry_days');
        $this->db->from('medicine_batches mb');
        $this->db->join('central_stocks cs', 'mb.id = cs.batch_id');
        $this->db->join('medicines m', 'mb.medicine_id = m.id');
        $this->db->join('medicine_brands mb2', 'm.brand_id = mb2.id');
    ...
    public function get_sale_items($sale_id) {
        $this->db->select('si.*, m.medicine_name, m.medicine_code, mb.brand_name as brand_name, mb2.batch_number, mb2.expiry_date');
        $this->db->from('sale_items si');
        $this->db->join('medicine_batches mb2', 'si.batch_id = mb2.id');
        $this->db->join('medicines m', 'mb2.medicine_id = m.id');
        $this->db->join('medicine_brands mb', 'm.brand_id = mb.id');
    ...
    /**
     * Get available batches for center-to-center transfer
     * @param int $from_center_id Source center ID
     * @param int $medicine_id Optional medicine filter
     * @return array
     */
    public function get_available_batches_for_transfer($from_center_id, $medicine_id = null) {
        try {
            $this->db->trans_start();
