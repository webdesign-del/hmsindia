...
$all_method =& get_instance();
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($select_result)){
            // INSERT INTO embryo_transfer_discharge_summary
            $query = "INSERT INTO `embryo_transfer_discharge_summary` SET " . implode(',' , $sqlArr);
        } else {
            // UPDATE embryo_transfer_discharge_summary
            $query = "UPDATE `embryo_transfer_discharge_summary` SET " . implode(',' , $sqlArr) . " WHERE patient_id='$patient_id'";
        }
        $result = run_form_query($query); 

        // 2. CHECK & UPDATE PCP_NDT TABLE (Duplicate Row Fix)
        $check_pcp = "SELECT * FROM `pcp_ndt` WHERE patient_id='$patient_id' AND test_type='ET/FET'";
        ...
        if(empty($pcp_result)){
           ...
           ('$patient_id', '".addslashes($wife_name)."', '".addslashes($husband_name)."', '".addslashes($wife_phone)."', '".addslashes($wife_age)."', '".addslashes($wife_address)."', 'P:".addslashes($female_pregnancy_other_p)."', 'L:".addslashes($female_pregnancy_other_l)."', 'A:".addslashes($female_pregnancy_other_a)."', '".addslashes($details_management_advised)."', '".addslashes($IVF_Consultant)."', '".addslashes($further_referredfor_dellvery)."', '".addslashes($outcome_of_pregnancy)."', '".addslashes($malformation_in_newborn)."', '".addslashes($center)."', 'ET/FET','IVF','" . date('Y-m-d H:i:s') . "')";
            ...
            $query2 = "UPDATE `pcp_ndt` SET 
                ...
                center = '$center'
                WHERE patient_id='$patient_id' AND test_type='ET/FET'";
        }
    ...
    $sql = "SELECT * FROM `embryo_transfer_discharge_summary` WHERE patient_id='$patient_id' and receipt_number='$receipt_number'";
    $select_result = run_select_query($sql);
