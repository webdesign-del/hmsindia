...
$all_method =& get_instance();
    ...
    if(isset($_POST['submit'])){
        ...
        // 1. Check if record already exists in iui_discharge_summary
        $sql = "SELECT * FROM `iui_discharge_summary` WHERE patient_id='$patient_id'";
        $select_result = run_select_query($sql);
        
        // Dynamic array query generator for iui_discharge_summary
        $sqlArr = array();
        ...
        if(empty($select_result)){
            // INSERT INTO iui_discharge_summary
            $query = "INSERT INTO `iui_discharge_summary` SET " . implode(',' , $sqlArr);
        } else {
            // UPDATE iui_discharge_summary
            $query = "UPDATE `iui_discharge_summary` SET " . implode(',' , $sqlArr) . " WHERE patient_id='$patient_id'";
        }
        $result = run_form_query($query); 

        // 2. CHECK & UPDATE PCP_NDT TABLE (Duplicate Row Fix)
        $check_pcp = "SELECT * FROM `pcp_ndt` WHERE patient_id='$patient_id' AND test_type='IUI'";
        ...
        if(empty($pcp_result)){
            ...
            $query2 = "UPDATE `pcp_ndt` SET 
                ...
                center = '$center'
                WHERE patient_id='$patient_id' AND test_type='IUI'";
        }
