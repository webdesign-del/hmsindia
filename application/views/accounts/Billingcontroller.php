	...
	public function consultation($appointment_id){
		...
		if($logg['status'] == true){
			...
			if(isset($_POST['action']) && isset($_POST['action']) && $_POST['action'] == 'add_consultation'){
				...
				$visit_month = date('F y', strtotime($_POST['on_date'])); // Output: "March 25"
				
				$only_date = date('Y-m-d', strtotime($_POST['on_date'])); // Output: "2025-03-13"
		...
		if($logg['status'] == true){
			...
			if(isset($_POST['action']) && isset($_POST['action']) && $_POST['action'] == 'add_registation'){
				...
				if($consult > 0){
					...
					$this->send_billing_receipt($biller_id, $paitent_id, $_POST['on_date'], $_POST['billing_from'], $_POST['receipt_number'], 'registation');
			...
			if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == "add_procedure"){
				...
				if(isset($_POST['procedure_suggestion']) && $_POST['procedure_suggestion'] == 1){
					...
					foreach($male_number as $key => $val){
						...
						//$procedure_array = [];
						//$post_arr['receipt_number'];
						//print_r($post_arr['receipt_number']);
						
						//$serialized_procedures = serialize($procedure_array);
						
						
						//echo $query = "INSERT INTO hms_patient_procedures_serialized (serialized_procedures) VALUES ('".$serialized_procedures."')";
