...
$(document).ready(function() {
    ...
    $('#add_return_item').click(function() {
        var newRow = '<tr>' +
            ...
            '<?php if(isset($available_batches) && !empty($available_batches)): ?>' +
            '<?php foreach($available_batches as $batch): ?>' +
            '<option value="<?php echo isset($batch->batch_id) ? $batch->batch_id : $batch->id; ?>" data-medicine="<?php echo isset($batch->medicine_name) ? $batch->medicine_name : 'Unknown'; ?>" data-price="<?php echo isset($batch->selling_price) ? $batch->selling_price : 0; ?>" data-sold-qty="<?php echo isset($batch->quantity_sold) ? $batch->quantity_sold : 0; ?>"><?php echo (isset($batch->medicine_name) ? $batch->medicine_name : 'Unknown') . ' - ' . (isset($batch->batch_number) ? $batch->batch_number : 'N/A'); ?></option>' +
            '<?php endforeach; ?>' +
            '<?php endif; ?>' +
            '</select></td>' +
            '<td><span class="batch_number">-</span></td>' +
