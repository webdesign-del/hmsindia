...
<div class="col-sm-12 col-xs-12  panel panel-piluku ml-5">
  <div class="row">
      <a href="<?php echo base_url('my_discharge');?>" class="btn btn-large pull-right">Go Back</a>
        ...
        <table class="table table-striped table-bordered table-hover dataList" id="">
          ...
          <tbody>
			<?php $procedure_billing = check_patient_procedure_billing($patient_id); //var_dump($procedure_billing);die;
				...
				$discharge_form_data = $all_method->get_patient_prodedures_data($appointment_id, $patient_id);
				foreach($discharge_form_data as $key => $patient_prodedures){
				  ...
				  foreach($procedure_datas['patient_procedures'] as $ky => $val){
					$procedure_assign = have_discharge_assign($val['sub_procedure'], 'daycare_procedure');
					if($procedure_assign){  ?>
				<tr>
					...
					<?php if(have_discharge_form($val['sub_procedure'], 'doctor')){ ?>	
						<a procedure_id="<?php echo $val['sub_procedure']; ?>" patient_procedure_id="<?php echo $patient_prodedures['ID']; ?>" href="javascript:void(0);" appointment_id="<?php echo $appointment_id; ?>" class="upload_discharge_record btn btn-large">Upload record</a>
						<div class="discharge_form_div" id="procedure_form_<?php echo $val['sub_procedure']; ?>"></div>
						<?php } ?>
					</td>
					<td style="border: 1px solid black; border-collapse: collapse;padding:5px; text-align:left;">
						...
						<?php $form_list = get_discharge_forms($val['sub_procedure']); //var_dump($form_list);die; 
							  $html = "";
							  foreach($form_list as $ks => $vls){
							    $form_details = get_discharge_form($vls['form_id']);
								if(isset($_SESSION['logged_doctor'])){
									if($form_details['form_for'] != "lab_procedure"){
										...
										if(count($check_form_data) > 0){
											$html .= "<a target='_blank' href='".base_url('check_discharge_form/'.$vls['form_id'].'/'.$patient_prodedures['ID'].'/'.$appointment_id.'/'.$val['sub_procedure'])."'>".$form_details['form_name']."</a> | ";
										}
									}
								}
							  }
...
$(document).on('click',".check_record",function(e) {
    $('.check_form_div').hide();
    $('.discharge_form_div').hide();
    $('.discharge_form_div').empty();
    var procedure_id = $(this).attr('procedure_id');
    $('#check_form_'+procedure_id).show();
});

$(document).on('click',".upload_discharge_record",function(e) {
	$('.check_discharge_form_div').hide();
	var procedure_id = $(this).attr('procedure_id');
	...
	$.ajax({
		url: '<?php echo base_url('doctors/discharge_upload')?>',
