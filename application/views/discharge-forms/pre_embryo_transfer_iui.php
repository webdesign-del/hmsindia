<?php
    ...
    // [FIX 2] यूआरएल से आने वाली डेट को सेफगार्ड करना
    $appoitmented_date = isset($_GET['appoitmented_date']) ? $_GET['appoitmented_date'] : '';

    // MySQL Database Insertion / Update Logic
    if(isset($_POST['submit']) && !empty($patient_id)){
        ...
        $sqlArr = array(); // [CRITICAL FIX] एरे को लूप से पहले हमेशा री-डिक्लेयर करें

        if(empty($select_result)){
            // INSERT Query: डेटाबेस में नया रिकॉर्ड सुरक्षित करने के लिए
            ...
            // UPDATE Query: [FIXED] पुराने रिकॉर्ड को सुरक्षित अपडेट करने के लिए
            $query = "UPDATE pre_embryo_transfer_iui SET ";
            foreach( $_POST as $key => $value ) {
                $sqlArr[] = " `$key` = '".addslashes($value)."'"; // addslashes यहाँ भी जोड़ा गया है
            }
            $query .= implode(',' , $sqlArr);
        ...
        if($result){
            // रीडायरेक्शन यूआरएल को क्लीन किया गया ताकि पुराना '?' या '&' सिंटैक्स न टूटे
            $redirect_url = strtok($_SERVER['HTTP_REFERER'], '?');
            header("location:" .$redirect_url."?patient_id=".$patient_id."&appoitmented_date=".$appoitmented_date."&m=".base64_encode('Discharge form saved successfully!').'&t='.base64_encode('success'));
            die();
        } else {
    ...
    // ==============================================================================
    // व्यू मोड (View Mode) के लिए डेटाबेस से डेटा खींचना
    // ==============================================================================
    $select_result = array();
    if (!empty($patient_id)) {
        if (!empty($appoitmented_date)) {
    ...
    // डिफ़ॉल्ट एरे कीज़ सेट करना ताकि HTML फ़ील्ड्स में Undefined Array Key एरर न आए
    if(empty($select_result)) {
        $select_result = array(
            'procedures' => '',
            'applicablemedicine' => ''
    ...
    // अप्वाइंटमेंट और सेंटर्स का डेटा सुरक्षित तरीके से निकालना
