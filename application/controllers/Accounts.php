		...
		if($logg['status'] == true){
			...
			if (isset($export_billing)){
				...
				foreach ($data as $key => $val) {//var_dump($val);die;
					...
					$billing_at = get_center_name($val['billing_at']);
					$lead_arr = array($val['patient_id'], $val['wife_name'], (string)$val['receipt_number'], $val['totalpackage'], $val['discounted_package'], $val['payment_done'], $val['remaining_amount'], $val['payment_method'], $billing_from, $billing_at, $val['billing_type'], $val['on_date'], $val['status']);
					fputcsv($fp, $lead_arr);
				}
				fclose($fp);
				exit();
...
foreach($ret_grouped as $return) {
   ...
   // $this->db->where('hms_consultation.status', 'approved');
    ...
    // =========================================================================
    $invest_rows = $this->db->query("SELECT * FROM hms_patient_investigations WHERE `status`='approved' AND `tally_status`='1' LIMIT 500")->result_array();
...
$consult_rows = $this->db->get()->result_array();
    ...
    // PART 5: INVESTIGATION SALES
    // =========================================================================
   $invest_rows = $this->db->query("SELECT * FROM hms_patient_investigations WHERE `status` IN ('approved', 'cancel') AND `tally_status` = '1' ORDER BY id DESC LIMIT 400")->result_array();
    foreach ($invest_rows as $sale) {
        $pt = $this->db->query("SELECT * FROM hms_patients WHERE patient_id = ?", [$sale["patient_id"]])->row_array();
        $bill_c = $this->db->query("SELECT * FROM hms_centers WHERE center_number = ?", [$sale["billing_at"]])->row_array();
        $org_c = $this->db->query("SELECT * FROM hms_centers WHERE center_number = ?", [$sale["origins"]])->row_array();
