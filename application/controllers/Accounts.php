    ...
    public function investigation_patients(){
		...
		if($logg['status'] == true){
			...
			if (isset($export_billing)){
				...
				$headers = 'IIC ID, Patient Name, Receipt Number, Total package, Discounted Package, Paid Amount, Remaining Amount, Payment Method, Billing From, Billing At, Billing Type, Date, Status, Origins';
				//Add the headers
				fwrite($fp, $headers. "\r\n");
				foreach ($data as $key => $val) {//var_dump($val);die;
					...
					$lead_arr = array($val['patient_id'], $val['wife_name'], $val['receipt_number'], $val['totalpackage'], $val['discounted_package'], $val['payment_done'], $val['remaining_amount'], $val['payment_method'], $billing_from, $billing_at, $val['billing_type'], date('Y-m-d H:i:s', strtotime($val['date'])), $val['status'],$origin_from);
					fputcsv($fp, $lead_arr);
				}
				fclose($fp);
				exit();
...
foreach($ret_grouped as $return) {
 ...
 $this->db->select('hms_consultation.*, hms_patients.wife_name, hms_patients.husband_name, 
	...
	$this->db->where('hms_consultation.on_date >', '2026-04-30');
    $this->db->limit(300);
	...
	$this->db->where('hms_registation.on_date >', '2026-04-30');
    $this->db->limit(300);
    ...
    // =========================================================================
    $invest_rows = $this->db->query("SELECT * FROM hms_patient_investigations WHERE `status`='approved' AND `tally_status`='1' AND `on_date` > '2026-04-30' ORDER BY id DESC LIMIT 500")->result_array();
...
foreach($ret_grouped as $return) {
    ...
    // PART 5: INVESTIGATION SALES
    // =========================================================================
    $invest_rows = $this->db->query("SELECT * FROM hms_patient_investigations WHERE `status` IN ('approved', 'cancel') AND `tally_status` = '1' AND `on_date` > '2026-04-30' ORDER BY id DESC LIMIT 500")->result_array();
 ...
 // =========================================================================
    ...
    // =========================================================================
    $partial_q = $this->db->query("SELECT * FROM hms_patient_payments WHERE status IN ('1', '3') AND tally_status='1' AND on_date > '2026-04-30' ORDER BY modified_on DESC LIMIT 500");
...
$consult_rows = $this->db->get()->result_array();
    ...
    // PART 5: INVESTIGATION SALES
    // =========================================================================
   $invest_rows = $this->db->query("SELECT * FROM hms_patient_investigations WHERE `status` IN ('approved', 'cancel') AND `tally_status` = '1' AND `on_date` > '2026-04-30' ORDER BY id DESC LIMIT 400")->result_array();
    foreach ($invest_rows as $sale) {
        $pt = $this->db->query("SELECT * FROM hms_patients WHERE patient_id = ?", [$sale["patient_id"]])->row_array();
        $bill_c = $this->db->query("SELECT * FROM hms_centers WHERE center_number = ?", [$sale["billing_at"]])->row_array();
        $org_c = $this->db->query("SELECT * FROM hms_centers WHERE center_number = ?", [$sale["origins"]])->row_array();
